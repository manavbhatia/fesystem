...
int main()
{
 ...
 int grid_no[max_grid_pt]={0}; // maximum grid points is 2500
 float grid_cord[max_grid_pt][3] = {{0.}}; //maximum grid points is 2500
 int n_grid_x, n_grid_y, n_grid_total; // number of grid points 
 float x_side_len,y_side_len; // plate side lengths
 float dx,dy; 
 float x_init = 0.,y_init= 0., angle=0.0;
...
// following statements calculate the data
 ...
 for (int i = 0; i < n_grid_y; i++ )
	...
	for (int j = 0; j < n_grid_x; j++)
		{ x_init = dx * float(j);
	        grid_cord[grid_number][0] = x_init;
		grid_cord[grid_number][1] = y_init;  	
		grid_cord[grid_number][2] = x_init*sin(angle*3.1416/180.0);  	
...
output_file << "E_11 27.0e9" << std::endl;
output_file << "NU 0.3" << std::endl;
...
{
	output_file << i+1	<< "   " << std::setprecision(10) << grid_cord[i][0]
	<< "   " << std::setprecision(10) << grid_cord[i][1] 
	<< "   " << std::setprecision(10) << grid_cord[i][2] << std::endl; 
}
...
output_file << "LOAD_SET_NAME PLATE_TRANSVERSE_LOAD" << std::endl;
output_file << "LOAD_SET_ID 103" << std::endl;
output_file << "LOAD_SET_KIND VOLUME_LOAD" << std::endl;
output_file << "LOAD_SET_N_DOFS 1" << std::endl;
elem_i = 1;
for (int i = 0; i < n_grid_y-1; i++ )
{
	for (int j = 0; j < n_grid_x-1; j++)
	{
		output_file << setw(8)<<elem_i++<<
		setw(8)<<"10.0"  << std::endl;
	}
}	
...
output_file << "BEGIN_LOAD_SET " << (n_grid_total+(n_grid_x+(n_grid_y-1))*6) << std::endl;
//output_file << "BEGIN_LOAD_SET " << ((n_grid_x+(n_grid_y-1))*6) << std::endl;
...
for (unsigned int j=1; j<=n_grid_x; j++)
{
	output_file << j << "  1  0.0" << std::endl; 
	output_file << j << "  2  0.0" << std::endl; 
	output_file << j << "  3  0.0" << std::endl; 
//	output_file << j << "  6  0.0" << std::endl; 

	output_file << (j+n_grid_x*(n_grid_y-1)) << "  1  0.0" << std::endl; 
	output_file << (j+n_grid_x*(n_grid_y-1)) << "  2  0.0" << std::endl; 
	output_file << (j+n_grid_x*(n_grid_y-1)) << "  3  0.0" << std::endl; 	
//	output_file << (j+n_grid_x*(n_grid_y-1)) << "  6  0.0" << std::endl; 	
}

for (unsigned int i=2; i<=(n_grid_y); i++)
{
	output_file << ((i-1)*n_grid_x+1) << "  1  0.0" << std::endl; 
	output_file << ((i-1)*n_grid_x+1) << "  2  0.0" << std::endl; 
	output_file << ((i-1)*n_grid_x+1) << "  3  0.0" << std::endl; 
//	output_file << ((i-1)*n_grid_x+1) << "  6  0.0" << std::endl; 
	
	output_file << ((i-1)*n_grid_x) << "  1  0.0" << std::endl; 
	output_file << ((i-1)*n_grid_x) << "  2  0.0" << std::endl; 
	output_file << ((i-1)*n_grid_x) << "  3  0.0" << std::endl; 	
//	output_file << ((i-1)*n_grid_x) << "  6  0.0" << std::endl; 	
}
...
output_file << "BOUNDARY_CONDITION_ID 104" << std::endl;
output_file << "N_TIME_INSTANTS 1" << std::endl;
output_file << "N_LOAD_SETS 3" << std::endl;
output_file << "TIME_INSTANT 0.0" << std::endl;
output_file << "NODAL_LOADS 102" << std::endl;
output_file << "NODAL_TEMPERATURE_LOAD 101" << std::endl;
output_file << "PLATE_DISTRIBUTED_TRANSVERSE_LOAD 103" << std::endl;
...
output_file << 	"INTEGER_SOLVER_PARAMETERS 1" << std::endl;
output_file << 	"NONLINEAR_SOLVER_MAXIMUM_ITERATIONS 20" << std::endl;
output_file << 	"REAL_SOLVER_PARAMETERS 1" << std::endl;
...
output_file << 	"DATA_VISUALIZER_INPUT_FILE  output_plots.gmv" << std::endl;
