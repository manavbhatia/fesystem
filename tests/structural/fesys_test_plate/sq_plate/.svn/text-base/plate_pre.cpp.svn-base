...
int main()
{
  ...
  cout << "\nType of element: 1 for tri, 2 for quad :";
  cin >> elem_type;
  ...
  int n_elem = 0;
  if (elem_type == 1)
    n_elem = (n_grid_y-1)*(n_grid_x-1) * 2;
  else 
    n_elem = (n_grid_y-1)*(n_grid_x-1);
      ...
      for (int j = 0; j < n_grid_x-1; j++)
        {
          if (elem_type == 1)
            {
              output_file << "STRUCTURAL_PLATE_DKT "	<<setw(8)<<elem_i++<<
            ...
            {
              output_file << "STRUCTURAL_PLATE_QUAD4 "	<<setw(8)<<elem_i++<<
              setw(8)<<"1"<<setw(8)<<n1<<setw(8)<<n1 + 1<<
              setw(8)<<n1+1+n_grid_x<<setw(8)<<n1+n_grid_x<<"\n"; 
            }
          n1 = n1 + 1;
  ...
  output_file << "N_LOADS " << n_elem << std::endl;
  elem_i = 1;
  unsigned int load_id=1;
  for (int i = 0; i < n_elem; i++ )
    {
      output_file << i+1 << "  " <<i+1<< "  3   " << 
  ...
  <<"N_LOAD_SETS 1 " << std::endl
  <<"DISPLACEMENT_BOUNDARY_CONDITION 1.0 4 " << std::endl
  <<"STATIC_LOAD_CASE END " << std::endl
  ...
  <<"STRUCTURAL_VIBRATION_EIGEN_SOLUTION BEGIN " << std::endl
  <<"ID 1 " << std::endl
  <<"GEOMETRIC_EFFECTS FALSE " << std::endl
  <<"COUPLED_THERMAL_STRUCTURAL FALSE " << std::endl
  <<"N_ANALYSIS_LOAD_CASES 1 " << std::endl
  <<"1 " << std::endl
  <<"N_SOLVER_INFO_ID 1 " << std::endl
  <<"EIGEN_SOLVER 2 " << std::endl
  <<"STRUCTURAL_VIBRATION_EIGEN_SOLUTION END " << std::endl
